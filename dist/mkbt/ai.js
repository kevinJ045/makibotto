export async function makiBottoAi(env, ctx) {
    ...
    if (env.DB) {
        const { success } = await env.DB
            ...
            .bind(crypto.randomUUID(), ctx.update.inline_query
            ? ctx.update.inline_query.from.id
            : ctx.update.message?.from.id, "[INST] " + _prompt + " [/INST]" + "\n" + response)
            .run();
        if (!success) {
